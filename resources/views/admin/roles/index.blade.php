    ...
    <div class="ibox box-primary">
        <div class="ibox-title">
            <div class="pull-right">
                {{-- @can('اضافة الرتب') --}}
                    <a href="{{ url('dashboard/role/create') }}" class="btn btn-primary">
                            ...
                            @foreach ($roles as $role)
                                <tr id="removable{{ $role->id }}">
                                    ...
                                    <td>
                                        ...
                                        <!-- Modal -->
                                        <div class="modal fade" id="client{{ $role->id }}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        ...
                                                        <h2 style="font-weight: bold" class="modal-title" id="myModalLabel">
                                                            {{ $role->name }}</h2>
                                                    </div>
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        @if ($role->permissions->isEmpty())
                                                            <h3 class="text-bold" style="text-align: center">
                                                                {{ __('لم يتم اختيار صلاحيات بعد') }} ..</h3>
                                                        @endif
                                                        @php
                                                            $title = '';
                                                        @endphp
    ...
    padding: 18px 0px;">{{ $perm->name }}</label>
                                                    ...
                                                    <div class="modal-footer">
                                                        <p><strong>{{ __('تاريخ الاضافة') }}
                                                                : </strong>
                                                            {{ $role->created_at->locale('ar')->isoFormat('dddd  , MMMM  ,  Do / YYYY  ,  الساعة h:mm') }}
                                                        </p>
                                    ...
                                    {{-- @can('تعديل الرتب') --}}
                                        <td class="text-center"><a href="{{ url('dashboard/role/' . $role->id . '/edit') }}"
                                                class="btn btn-xs btn-success"><i class="fa fa-edit"></i></a></td>
                                    {{-- @endcan --}}
                                    {{-- @can('حذف الرتب') --}}
                                        <td class="text-center">
                                            <button id="{{ $role->id }}" data-token="{{ csrf_token() }}"
                                                data-route="{{ url('dashboard/role/' . $role->id) }}" type="button"
